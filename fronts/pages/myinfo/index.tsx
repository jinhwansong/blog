...
const myinfo = ()=>{
    ...
    const onChangeProfile = useCallback((e:React.ChangeEvent<HTMLInputElement>)=>{
      if (e.target.files && e.target.files[0]) {
        if (e.target.files[0].name.length > 30) return alert("글자수를 30글자 미만으로 적어주세요")
    ...
    return (
      <>
        <Head>
          ...
          <meta name="og:title" content={`${me.name} 님의 개인정보`} />
          <meta name="og:description" content={`${me.name} 님의 개인정보`} />
          <meta
            name="og:image"
